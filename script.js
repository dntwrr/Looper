...
function PasteLogin() {
    link = $(`<a href='https://anilist.co/api/v2/oauth/authorize?client_id=${id}&response_type=token'>`);
    link.text('Login');
    link.attr("id","Login")
    $('body').append(link);
};   
...
function createTable(k) {
  ...
  const row2=$('<div id="2" class="col-12 row">')
  const row3=$('<div id="3" class="col-12 row">')
  let anime = data[k].media;
  
  const name = $('<p class="col-6">');
  ...
  const description = $('<textarea class="col-5 descript">');
